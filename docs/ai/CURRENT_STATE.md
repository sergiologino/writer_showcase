...
- Маршруты: `/login`, `/register`, `/app/feed`, `/app/profile` (**админ:** блок настройки нейросетей при `isAdmin`), `/app/posts/new`, `/app/posts/:id` (редактор: **AI-студия**, плановая дата, предупреждение при `channelSyndicationBlocked`), публично `/blog/:workspaceSlug`, `/blog/:workspaceSlug/p/:postSlug`.
